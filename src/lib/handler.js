const { ReactionCollector, Message } = require('discord.js');
/**
...
class ReactionHandler extends ReactionCollector {
	...
	constructor(message, filter, options, display, emojis) {
		...
		this.on('collect', (reaction, user) => {
			reaction.users.cache.forEach(m => {
                if(!m || m.id === this.client.user.id) return null;
				reaction.users.remove(m.id).catch(() => null)
			});
		...
		this.on('end', () => {
			if (this.reactionsDone && !this.message.deleted) 
                ...
                .then(() => {
				    if(!this.message.deleted) this.message.delete({ timeout: 2000 }).catch(() => {});
				}).catch(() => null)
		});
	}
	 ...
	 */
	info() {
		this.message.edit(this.display.infoPage).catch(() => null)
	}
	 ...
	 */
	update() {
		this.message.edit('', { embed: this.display.pages[this.currentPage] }).catch(() => {});
	}
	...
	async _queueEmojiReactions(emojis) {
		...
		if (this.ended) return this.message.reactions.removeAll().catch(() => {});
		await this.message.react(emojis.shift()).catch(() => {});
